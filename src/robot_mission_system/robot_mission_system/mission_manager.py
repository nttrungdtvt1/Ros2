...
#         self.get_logger().info(f'mission_manager command topic {self._command_topic}')
...
#             data = json.loads(msg.data)
#         except json.JSONDecodeError:
#             self.get_logger().warning(f'Invalid JSON command: {msg.data}')
#             return
#         cmd = str(data.get('cmd', '')).lower()
...
#             self._repo.reload()
#         else:
#             self.get_logger().warning(f'unknown cmd {cmd}')
...
#         ok, detail = self._nav.send_pose(pose)
#         if not ok:
#             self.get_logger().error(f'send_pose failed: {detail}')
#             self._fail_streak += 1
#             if self._fail_streak >= self._max_fails:
#                 self._state = 'FAILED'
#             return
#         self._goal_deadline = self.get_clock().now().nanoseconds / 1e9 + self._goal_timeout
#         self.get_logger().info(f"Navigating to '{name}' ({self._idx + 1}/{len(self._queue)})")
...
#             return
#         status, detail = res
#         self.get_logger().info(f'Nav result {detail}')
#         if status == GoalStatus.STATUS_SUCCEEDED:
#             self._fail_streak = 0
#             if self._use_precision:
#                 rec = self._repo.get(self._queue[self._idx])
...
#         else:
#             self._fail_streak += 1
#             self.get_logger().warning(f'Nav did not succeed (status={status})')
#             if self._fail_streak >= self._max_fails:
#                 self._state = 'FAILED'
#             else:
#                 self._start_current_waypoint()
...
class MissionManagerNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter('max_consecutive_failures', 3)
        self.declare_parameter('require_scan_before_goal', True)
        self.declare_parameter('precision_phase_timeout_sec', 30.0)
        ...
        # CHUẨN HÓA QoS AMCL: Đảm bảo không nghẽn mạng trên DDS
        amcl_qos = QoSProfile(
        ...
        self._require_scan = self.get_parameter('require_scan_before_goal').get_parameter_value().bool_value
        self._max_fails = self.get_parameter('max_consecutive_failures').get_parameter_value().integer_value
        self._precision_timeout = self.get_parameter('precision_phase_timeout_sec').get_parameter_value().double_value
        ...
        self.get_logger().info(f'mission_manager initialized, listening to {self._command_topic}')

    def _on_amcl(self, msg: PoseWithCovarianceStamped) -> None:
        # COVARIANCE GUARD: Bộ lọc phương sai, bỏ qua Yaw nhiễu
        cov_yaw = msg.pose.covariance[35]
        if cov_yaw > 0.05:
            self.get_logger().warning(f'AMCL yaw uncertainty too high ({cov_yaw:.3f}), waiting for convergence...', throttle_duration_sec=2.0)
            return
    ...
    def _start_current_waypoint(self) -> None:
        ...
        if self._require_scan and not self._obstacle.scan_ok():
            self.get_logger().warning('Waiting: Lidar scan not healthy', throttle_duration_sec=2.0)
            self._waypoint_send_pending = True
            return
    ...
    def _tick_nav(self) -> None:
        # LOGIC STATE MACHINE: Được làm lại hoàn toàn để xử lý Retry mượt mà
        if self._state != 'RUNNING':
            return

        # 1. Bỏ qua nếu đang bận đi
        if self._nav.has_active_goal:
            now = self.get_clock().now().nanoseconds / 1e9
            if self._goal_deadline is not None and now > self._goal_deadline:
                self.get_logger().warning('Goal timeout; cancel and retry same leg')
                self._nav.cancel()
        ...
        # 2. Xử lý Retry nếu hệ thống vừa phục hồi từ Lidar Fail
        if self._waypoint_send_pending:
            self._start_current_waypoint()
            return

        # 3. Phân tích kết quả sau khi đến Goal
        res = self._nav.poll_result()
        if res is None:
            return
        ...
        if status == GoalStatus.STATUS_SUCCEEDED:
            ...
            if self._use_precision:
                ...
                if rec is not None:
                    self._precision_target_yaw = quaternion_to_yaw(rec.qx, rec.qy, rec.qz, rec.qw)
                    self._state = 'PRECISION'
                    self._precision_deadline = (self.get_clock().now().nanoseconds / 1e9) + self._precision_timeout
                else:
                    self._advance_queue()
            else:
                self._advance_queue()
