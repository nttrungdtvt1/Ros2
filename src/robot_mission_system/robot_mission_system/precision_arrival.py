...
# Copyright 2026 Robot Mission System
"""Final approach: in-place yaw alignment via cmd_vel_precision with Smooth Decay Controller."""
...
class PrecisionArrival:
    ...
    def align_yaw_step(self, current_yaw: float, target_yaw: float) -> tuple[Twist, bool]:
        ...
        # 1. Deadband hoàn hảo: Dừng hẳn khi đã nằm trong dung sai
        if abs_err < self._yaw_tolerance:
            return Twist(), True
        ...
        # Tính toán vận tốc dựa trên hệ số P, giới hạn bằng max_omega
        velocity = min(self._max_omega, abs_err * Kp)

        # 2. Vùng hội tụ êm (Smooth Decay): 
        # Chỉ kích hoạt min_omega khi góc lệch còn lớn, giúp motor thắng ma sát tĩnh.
        # Khi tiến sát đích (abs_err <= 2*tolerance), cho phép vận tốc giảm dần về 0 một cách tuyến tính.
        if velocity < self._min_omega and abs_err > (2.0 * self._yaw_tolerance):
            velocity = self._min_omega

        cmd.angular.z = sign * velocity
        return cmd, False
